...
export default function TaskForm({ users, handleCreateTask }) {
			...
			<Grid container>
				...
				<Grid item xs={4} className={classes.mxAuto}>
					<IconButton aria-controls="simple-menu" aria-haspopup="true" onClick={handleClick}>
						{assigned_to ? <Avatar alt={users[assigned_to].name} src={users[assigned_to].picture} /> : <AccountCircle />}
					</IconButton>
					<Menu id="simple-menu" anchorEl={anchorEl} keepMounted open={Boolean(anchorEl)} onClose={handleClose}>
						{renderUsers()}
					</Menu>
					<IconButton aria-label="add">
						<PriorityHighRounded color={selectPriorityColor()} onClick={handleClicka1} />
						<Menu id="simple-menu" anchorEl={anchorEl1} keepMounted open={Boolean(anchorEl1)} onClose={handleClose1}>
							{renderPriorities()}
						</Menu>
					</IconButton>
					<IconButton aria-label="add">
						<AddOutlined onClick={() => handleCreateTask(form)} />
					</IconButton>
				</Grid>
			</Grid>
		</div>
